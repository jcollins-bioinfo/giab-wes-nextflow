...
def build_domains(target,truth,dictionary,out,run_id,padding=100):
 ...
 return result
def main():
 p=argparse.ArgumentParser();p.add_argument("--workspace",required=True);p.add_argument("--run-id",required=True);p.add_argument("--target-bed");p.add_argument("--source-dict");p.add_argument("--picard",default="picard");p.add_argument("--min-liftover-pct",type=float,default=.95);a=p.parse_args();root=Path(a.workspace);gate=json.loads(config_path("m2-target-design.json").read_text())

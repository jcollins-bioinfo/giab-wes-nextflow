...
output "healthomics_execution_role_arn" { value = try(aws_iam_role.project["healthomics-execution"].arn, null) }
output "batch_job_role_arn" { value = try(aws_iam_role.project["batch-job"].arn, null) }
output "batch_benchmark_job_role_arn" { value = try(aws_iam_role.project["batch-benchmark-job"].arn, null) }
output "batch_execution_role_arn" { value = try(aws_iam_role.project["batch-execution"].arn, null) }

"""Protect M2 package resources, fail-closed manifests and thin notebook launchers."""
...
class HotfixTest(unittest.TestCase):
 ...
 def test_manifest_rejects_malformed_duplicate_legacy_and_missing(self) -> None:
  """Reject malformed, duplicated, legacy and unsafe source manifests."""
 ...
 def test_notebook_is_clean_and_current(self) -> None:
  """Keep the notebook output-free and bound to package-owned launch interfaces."""
  notebook=json.loads((Path(__file__).resolve().parents[2]/'notebooks/m2_colab.ipynb').read_text());source='\n'.join(''.join(x['source']) for x in notebook['cells'])

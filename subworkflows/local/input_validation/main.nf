workflow INPUT_VALIDATION {
    ...
    normalized = raw_rows.map { rows ->
        ...
        rows.collect { row ->
            ...
            required.each { if (!row[it]?.toString()?.trim()) error "Missing required field: ${it}" }
            ['sample','library','lane','read_group_id','platform_unit'].each {
                if (!(row[it] ==~ /[A-Za-z0-9][A-Za-z0-9._-]*/)) error "Identifier is not URL-safe: ${it}"
            }
            ...
            def key = required.collect { row[it] }.join('\u001f')
